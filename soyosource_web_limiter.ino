...
String processor(const String& var){
   ...
   DEBUG_SERIAL.println(var);
    ...
    if(var == "STATICPOWER"){
      ...
      //return uptimeString.getUptime2();
      return Uptime_Str;
    }
...
void myUptime(){
  ...
  if (Uptime_Years == 0U) {                                  // Uptime Is Less Than One Year
    ...
    // First Day And Less Than Month
      ...
      sprintf(Uptime_Str, "%i day-%02i:%02i:%02i", Uptime_Days, Uptime_Hours, Uptime_Minutes, Uptime_Seconds);
    // Second Day And More But Less Than Month
    else if (Uptime_Days >= 2U && Uptime_Months == 0U)
      sprintf(Uptime_Str, "%i days-%02i:%02i:%02i", Uptime_Days, Uptime_Hours, Uptime_Minutes, Uptime_Seconds);
    // First Month And More But Less Than One Year
    else if (Uptime_Months >= 1U)
      sprintf(Uptime_Str, "%im, %id-%02i:%02i:%02i", Uptime_Months, Uptime_Days, Uptime_Hours, Uptime_Minutes, Uptime_Seconds);
    // If There Is Any Error In This If Loop Then Make Full String.
    else sprintf(Uptime_Str, "%iy %im %id-%02i:%02i:%02i", Uptime_Years, Uptime_Months, Uptime_Days, Uptime_Hours, Uptime_Minutes, Uptime_Seconds);
  } else                                                     // Uptime Is More Than One Year
    sprintf(Uptime_Str, "%iy %im %id-%02i:%02i:%02i", Uptime_Years, Uptime_Months, Uptime_Days, Uptime_Hours, Uptime_Minutes, Uptime_Seconds);
...
void loop() {
  ...
  //send hello to SoyoSource every 1000ms
  if ((millis() - lastTime2) > timerDelay2) {
    ...
    //events.send(uptimeString.getUptime2(),"uptime", millis());
    myUptime();
